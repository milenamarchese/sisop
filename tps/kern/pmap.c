...
boot_alloc(uint32_t n)
{
	...
	if (!nextfree) {
        ...
        cprintf("boot_alloc: initialized in %x\n", nextfree);
	}
    ...
    cprintf("boot_alloc: allocating %d bytes (%d pages), starting in %x\n", n, n / PGSIZE, nextfree);
	if (n == 0) {
		return nextfree;
	}
    ...
    cprintf("boot_alloc: allocation ends in %x\n", nextfree);
...
mem_init(void)
{
    ...
    cprintf("mem_init: kern_pgdir allocated in %x\n", kern_pgdir);
    ...
    cprintf("mem_init: pages allocated in %x\n", pages);
    ...
    cprintf("mem_init: mapping pages (va[%08lx, %08lx] -> pa[%08lx, %08lx])\n",
            UPAGES, UPAGES + size,
            PADDR((void *)pages), PADDR((void *)pages) + size
    );
    boot_map_region(kern_pgdir, (uintptr_t)UPAGES, size, PADDR((void *)pages), PTE_U);
	...
	// LAB 3: Your code here.
    cprintf("mem_init: mapping pages (va[%08lx, %08lx] -> pa[%08lx, %08lx])\n",
            UENVS, UENVS + envs_size,
            PADDR((void *)envs), PADDR((void *)envs) + envs_size
    );
    boot_map_region(kern_pgdir, (uintptr_t)UENVS, envs_size, PADDR((void *)envs), PTE_U|PTE_P);
    ...
    cprintf("mem_init: mapping kernel stack (va[%08lx, %08lx] -> pa[%08lx, %08lx])\n",
            KSTACKTOP-KSTKSIZE, KSTACKTOP-KSTKSIZE + size,
            PADDR(bootstack), PADDR(bootstack + size)
    );
    boot_map_region(kern_pgdir, (uintptr_t)KSTACKTOP-KSTKSIZE , size, (physaddr_t)PADDR(bootstack), PTE_W);
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here:
    size = (1ULL << 32) - KERNBASE;
    cprintf("mem_init: mapping physical memory (va[%08lx, %08lx] -> pa[%08lx, %08lx])\n",
            KERNBASE, KERNBASE + size,
            0, size
    );
    boot_map_region(kern_pgdir, (uintptr_t)KERNBASE, size, 0, PTE_W);
...
page_init(void)
{
    ...
    int begin = PGNUM(IOPHYSMEM);
    int end = PGNUM(PADDR(boot_alloc(0)));

    for (size_t i = 1; i < npages; i++) {

        if ((i >= begin && i < end) || i == 7) {
            continue;
        }
...
{
    if (pp->pp_ref) panic("page_free: The page still has references!");
    if (pp->pp_link) panic("page_free: There is still a reference to the next free page!");
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if (!pgdir[index]) {
		...
		page->pp_ref++;
		pgdir[index] = page2pa(page) | PTE_P | PTE_W | PTE_U;
	}

	pde_t* page_table = KADDR(PTE_ADDR(pgdir[index]));

	return page_table + entry;
}
//
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
    size_t i = 0;
    while (i < size) {
        int pperm = perm | PTE_P;
        size_t inc = PGSIZE;
#ifndef TP1_PSE
        pte_t *entry = pgdir_walk(pgdir, (void *)va + i, 1);
#else
        pte_t *entry;
        if ((size - i >= PTSIZE) && (PTX(pa + i) == 0) && (PTX(va + i) == 0)) {
            inc = PTSIZE;
            pperm |= PTE_PS;
            entry = pgdir + PDX(va + i);
        } else {
            entry = pgdir_walk(pgdir, (void *)va + i, 1);
        }
#endif
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for (uint32_t address = start; address < end; address += PGSIZE) {
		if (address >= ULIM) {
			user_mem_check_addr = address < (uint32_t) va ? (uint32_t) va : address;
			return -E_FAULT;
		}
