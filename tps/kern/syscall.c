...
sys_page_alloc(envid_t envid, void *va, int perm)
{
    ...
    if ((check_perm(perm) < 0) || (check_mem(va) < 0))
        return -E_INVAL;
...
sys_page_map(envid_t srcenvid, void *srcva, envid_t dstenvid, void *dstva, int perm)
{
    ...
    if ((check_mem(srcva) < 0) || (check_mem(dstva) < 0))
        return -E_INVAL;

    pte_t *pte_store;
    struct PageInfo *page_info = page_lookup(srcenv->env_pgdir, srcva, &pte_store);
    if (!page_info) return -E_INVAL;
    if ((perm & PTE_W) && ((*pte_store & PTE_W) == 0)) return -E_INVAL;

    if(page_insert(dstenv->env_pgdir, page_info, dstva, perm) < 0) {
        return -E_NO_MEM;
    }
    return 0;
}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
    ...
    if (srcva < (void*)UTOP) {     
        if (((uintptr_t)srcva % PGSIZE != 0) || (check_perm(perm) < 0))
            return -E_INVAL;
