...
trap_dispatch(struct Trapframe *tf)
{
    ...
    switch (tf->tf_trapno) {
        ...
        case T_PGFLT:
            ...
            return;
        case T_SYSCALL:
            result = syscall(tf->tf_regs.reg_eax, tf->tf_regs.reg_edx, 
...
page_fault_handler(struct Trapframe *tf)
{
	...
	if (((tf->tf_cs & 3) == 0) && (tf->tf_trapno == T_PGFLT)){
    ...
    uintptr_t dir = UXSTACKTOP;

    if (tf->tf_esp >= UXSTACKTOP - PGSIZE && tf->tf_esp < UXSTACKTOP) {
        // stack was already the user exception stack, wich means we are in a recursive exception
        dir = tf->tf_esp - 4;
    }

    struct UTrapframe *u = (struct UTrapframe *)(dir - sizeof(struct UTrapframe));
