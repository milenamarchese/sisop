...
static char* expand_environ_var(char* arg) {
	...
	if (arg_len == 2 && arg[1] == '?') {
		char last_stat[2] = {'0' + status, '\0'};
		return strcpy(arg, last_stat);
	}

	char* env_val = getenv(&arg[1]);

	if (env_val == NULL) {
		return strcpy(arg, EMPTY);
	 }

	size_t env_val_size = strlen(env_val);

	if (env_val_size > strlen(arg)) {
		char* new_arg = realloc(arg, (env_val_size + 1) * sizeof(char));
		return strcpy(new_arg, env_val);
	}
	return strcpy(arg, env_val);
}
...
struct cmd* parse_line(char* buf) {
	...
	struct cmd** commands = calloc(sep + 2, sizeof(struct cmd*));

	for (int i = 0; i < sep + 1; i++) {
		char * right = split_line(buf, '|');
		commands[i] = (struct cmd*) parse_cmd(buf);
		buf = right;
	}

	commands[sep + 1] = NULL;

	return pipe_cmd_create(commands);
}
